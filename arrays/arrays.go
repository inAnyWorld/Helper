...
// InArrayInteger 是否在数组内
func InArrayInteger(integer int64, arr []int64) (inArray bool) {
	for _, i := range arr {
		if i == integer {
			inArray = true
			break
...
// StrInArr 字符串是否在字符数组中
func StrInArr(target string, strArr []string) bool {
	sort.Strings(strArr)
...
func (ka *LkkArray) ArrayUnique(arr interface{}) []interface{} {
	...
	case reflect.Array, reflect.Slice:
		for i := 0; i < val.Len(); i++ {
			...
			key = string(gostring.Md5Str([]byte(str), 32))
			if _, ok := mp[key]; !ok {
				mp[key] = true
				res = append(res, item)
			}
